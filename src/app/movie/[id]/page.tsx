...
export default function MovieDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [movie, setMovie] = useState<CatalogItem | null>(null);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      savePlaybackProgress(movie.id, Math.min(98, 10), {
        title: movie.title, type: "movie", posterPath: movie.posterUrl || "", backdropPath: movie.backdropUrl || "", imdbId: movie.imdbId
      });
    }, 5000);
      ...
      <div className={`${theater ? "w-full" : "max-w-[1400px] mx-auto w-full px-6 sm:px-12"}`}>
        {playing ? (
          ...
          <div className="relative aspect-video sm:aspect-[21/9] rounded-xl overflow-hidden border border-white/5 group shadow-2xl">
            <img src={movie.backdropUrl || movie.posterUrl} alt={movie.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-700"
      ...
      <div className="max-w-[1400px] mx-auto w-full px-6 sm:px-12 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
        <div className="flex flex-col gap-4">
          <div className="flex flex-wrap gap-2">
            {movie.genres.map(g => <span key={g} className="text-xs font-semibold text-white/60 bg-white/5 px-3 py-1 rounded-md border border-white/5">{g}</span>)}
          </div>
          ...
          <div className="flex items-center gap-3 text-sm text-white/60 font-medium">
            <span className="flex items-center gap-1 text-yellow-400 font-bold"><Star size={14} fill="currentColor" /> {movie.vote_average}</span>
            <span className="flex items-center gap-1"><Clock size={13} /> {movie.runtime}</span>
            <span>{movie.release_date?.split("-")[0]}</span>
          </div>
          <p className="text-[15px] text-white/70 leading-relaxed">{movie.overview}</p>
          {movie.cast && movie.cast.length > 0 && (
            <div className="border-t border-white/5 pt-4 mt-2">
              <h3 className="text-xs font-bold uppercase text-white/40 mb-2 tracking-wider">Cast</h3>
              <div className="flex flex-wrap gap-2">
                {movie.cast.map(a => <span key={a} className="text-xs font-medium text-white/60 bg-white/5 px-2.5 py-1 rounded-md">{a}</span>)}
              </div>
            </div>
          )}
        </div>
